...
**Keyboard — fly the camera:** `W`/`S` forward/back · `A`/`D` strafe left/right ·
`G` rotate the 3D view right · `F` rotate it left. Movement speed scales with
altitude, so one tap feels proportional whether you are at orbit or at street
level. Held keys move per animation frame, not per key-repeat, so motion is
smooth and machine-independent.

**Keyboard — everything else:** `1`–`7` visual styles · `H` HUD · `L` layer
panel · `X` detection · `C` cockpit · `O` orbit · `V` clean view · `Esc` out.

> **Moved on purpose:** the layer panel was `F` and detection was `D`. Both
> letters now drive the camera, and one keypress must never both move the view
> and toggle a panel.

### 🇮🇩 Build Indonesia

Build ini berbeda dari hulu dalam tiga hal:

- **Antarmuka berbahasa Indonesia.** Template diterjemahkan saat build dari
  kamus tunggal di [`src/ui/i18n/id.json`](src/ui/i18n/id.json), sehingga
  templatnya sendiri tetap berbahasa Inggris dan tetap mudah digabung dengan
  hulu. Nama penyedia data, singkatan teknis (HUD, CCTV, FOV, NVG), dan ligatur
  ikon Material sengaja tidak diterjemahkan.
- **Tanpa bingkai lingkaran.** Masker teropong melingkar (yang membuat layar
  tampak seperti mata) kini **mati secara bawaan** — globe tampil penuh dan HUD
  tetap di atasnya. Tombol **Teropong** di panel Tampilan masih menyalakannya,
  begitu pula tautan berbagi dengan `sc=1`.
- **CCTV Indonesia.** 250 kamera publik dari Jakarta, Bandung, dan Yogyakarta,
  lengkap dengan proxy HLS dan pemutar `hls.js` agar streamnya benar-benar
  tampil di Chrome dan Firefox, bukan hanya Safari.
...
| 📹 **CCTV Mesh** | ~3,600 public cameras projected *into* the 3D space — Austin · Texas (TxDOT) · California (Caltrans) · London (TfL) · Ontario (511) · Finland (Fintraffic) · British Columbia (DriveBC) · Estonia (Tallinn, Tarktee) · New South Wales (Live Traffic NSW) · Calgary · **Indonesia — Jakarta · Bandung · Yogyakarta (250 cameras, live HLS)**. Positions are published; poses are estimated priors **you calibrate by dragging a gizmo on the camera itself** (Jakarta's positions are geocoded estimates too — see [DATA_SOURCES.md](DATA_SOURCES.md)) | City APIs | 🟢 |
