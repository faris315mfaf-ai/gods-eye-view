      ...
      try {
        ...
        if (url.pathname.startsWith('/media/')) {
          const cameraId =
            ...
            'camera';
          const source = sourceById.get(cameraId);
          const registeredUrl = source?.url || '';
          const feedType = normalizeFeedType(source?.feedType || 'image');

          // `?u=` is set only by this proxy's own playlist rewriting (hls.js):
          // an HLS playlist names segments and variant playlists, and each of
          // those comes back here as a sub-request. The URL is caller-supplied
          // and therefore untrusted — it is accepted only when it resolves to
          // the same origin as the camera's REGISTERED stream URL, so this
          // parameter can never aim the proxy at another host.
          const requestedSubResource = url.searchParams.get('u');
          const allowedMediaOrigin = mediaOriginFor(registeredUrl);
          let mediaUrl = registeredUrl;
          if (requestedSubResource) {
            const resolved = resolveHlsSegmentUrl(
            ...
            if (!resolved) {
              ...
              res.end(
                JSON.stringify({
                  error: 'Requested media is not on this camera origin',
                }),
              );
              return;
            }
          ...
          try {
            ...
            // An HLS playlist is an index, not media. Every URI inside it is
            // rewritten to come back through this proxy before the player sees
            // it — otherwise relative segment names resolve against this
            // server's path and the stream dies on its first hop. See hls.js.
            if (isHlsPlaylistResponse(contentType, mediaUrl)) {
              const playlist = await readResponseTextCapped(
                upstream,
                CCTV_PLAYLIST_MAX_BYTES,
              );
              const { body, rewritten, dropped } = rewriteHlsPlaylist(
                playlist,
                {
                  cameraId,
                  baseUrl: mediaUrl,
                  allowedOrigin: allowedMediaOrigin,
                },
              );
              setHealth(cameraId, {
                status: rewritten > 0 ? 'ok' : 'degraded',
                sourceKind: 'live',
                label: source?.provider || 'Configured source',
                message:
                  rewritten > 0
                    ? `HLS playlist proxied (${rewritten} URIs${dropped ? `, ${dropped} off-origin dropped` : ''})`
                    : 'HLS playlist contained no playable URIs',
              });
              res.writeHead(200, {
                'Content-Type': 'application/vnd.apple.mpegurl',
                'Cache-Control': 'no-store',
                'X-CCTV-Source': 'live-media-playlist',
              });
              res.end(body);
              return;
            }
