...
/**
 * Indonesia's national bounding box, from Sabang in the northwest to the
 * Papua border in the east and Rote in the south. Wide on purpose: the ATCS
 * packs are per-city, but the curated catalog is national, and this only has
 * to reject a null island or a lat/lon the portal published swapped.
 */
export function isLikelyIndonesiaCoordinate(lat, lon) {
  return (
    isPlausibleLatLon(lat, lon) &&
    lat >= -11.5 &&
    lat <= 6.5 &&
...
/** DKI Jakarta province, from the Thousand Islands ferry coast to Depok. */
export function isLikelyJakartaCoordinate(lat, lon) {
  return (
    isPlausibleLatLon(lat, lon) &&
    lat >= -6.4 &&
    lat <= -6.05 &&
...
/** Yogyakarta city and its immediate ring road. */
export function isLikelyJogjaCoordinate(lat, lon) {
  return (
    isPlausibleLatLon(lat, lon) &&
    lat >= -8.1 &&
    lat <= -7.6 &&
...
/** Bandung city and the Cimahi/Gedebage corridor its ATCS covers. */
export function isLikelyBandungCoordinate(lat, lon) {
  return (
    isPlausibleLatLon(lat, lon) &&
    lat >= -7.05 &&
    lat <= -6.75 &&
