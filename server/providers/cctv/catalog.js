...
const LIVE_PACKS = [
  ...
  // Indonesia: one pack per city, because each city publishes through its own
  // portal on its own stack — the ATCS sites run by each Dinas Perhubungan,
  // and the DKI provincial CCTV wall for Jakarta. All are keyless, and each is
  // gated independently so one city's outage never takes the others down.
  {
    name: 'jogja',
    enabled: () => envEnabled('CCTV_JOGJA_ENABLED'),
    load: loadJogjaSourcesFromAtcs,
  },
  {
    name: 'jakarta',
    enabled: () => envEnabled('CCTV_JAKARTA_ENABLED'),
    load: loadJakartaSourcesFromPortal,
  },
  {
    name: 'bandung',
    enabled: () => envEnabled('CCTV_BANDUNG_ENABLED'),
    load: loadBandungSourcesFromAtcs,
  },
  {
    name: 'indonesia-curated',
    enabled: () => envEnabled('CCTV_INDONESIA_CURATED_ENABLED'),
    load: loadIndonesiaCuratedSources,
  },
];
/**
