...
// ---------------------------------------------------------------------------
// 1b. Scope mask OFF — the plain full-frame globe
// ---------------------------------------------------------------------------

test('first run opens WITHOUT the circular scope, at every surface that decides it', () => {
  // This build ships the plain full-frame globe: the circular treatment reads
  // as looking through an eye/telescope, and the HUD is meant to sit over an
  // uncropped scene. The toggle and the sc=1 share-link field still enable it.
  assert.equal(isScopeMaskEnabled(), false,
    'scopeMask.js: the live module starts disabled');

  assert.match(indexHtml, /id="scope-toggle"[^>]*aria-pressed="false"/,
    'index.html: the toggle ships unpressed');
  assert.doesNotMatch(indexHtml, /class="pp-toggle-btn active" id="scope-toggle"/,
    'index.html: and carries no active class');

  assert.match(shareSource, /this._scopeEnabled = false;/,
    'sharelink.js: the generator starts from the same state the mask starts in');
  // A link with no sc field follows the build default rather than re-enabling it.
