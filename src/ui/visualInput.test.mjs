...
test('letter shortcuts retain uppercase handling and existing actions', () => {
  const f = shortcuts();
  // 'f' -> 'l' and 'd' -> 'x': both letters now drive the keyboard camera
  // controls (keyboardFlight.js), so the panel shortcuts moved off them.
  for (const key of ['H', 'o', 'V', 'L', 'X', 'c']) f.press(key);
  assert.deepEqual(
    f.calls,
    [
      'toggleHud',
...
for (const tag of ['INPUT', 'SELECT', 'TEXTAREA']) {
  test(`${tag} retains native editing, while Escape reaches search dismissal`, () => {
    const f = shortcuts();
    for (const key of ['1', 'h', 'o', 'v', 'l', 'x', 'c'])
