...
export class ShareLinkManager {
  constructor(viewer, {
    ...
    // OFF by default in this build — see the note on `_enabled` in scopeMask.js.
  ...
  parseInitialHash() {
    ...
    const state = {
      ...
      // Fallback follows the build default (OFF). A link that wants the scope
      // carries `sc=1` explicitly, because the generator always writes the field.
