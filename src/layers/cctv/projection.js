...
import { attachVideoSource } from './hlsPlayback.js';
...
export function createProjection({
  ...
  function createProjectionRuntime(record) {
    ...
    const runtime = {
      mode,
      // Set when a video feed is attached; tears down an hls.js player if one
      // was created for this camera.
      detachVideo: null,
      canvas,
      ctx,
      image: null,
    ...
    if (mode === 'video') {
      ...
      // HLS needs a player, not a src: see hlsPlayback.js. MP4/WebM still take
      // the plain assignment path inside this helper.
      runtime.detachVideo = attachVideoSource(
        video,
  ...
  function destroyProjectionRuntime(runtime) {
    ...
    if (runtime.video) {
      runtime.video.pause();
      // Tears down an hls.js player when one was attached, and clears the
      // element either way.
      if (runtime.detachVideo) {
        runtime.detachVideo();
        runtime.detachVideo = null;
      } else {
