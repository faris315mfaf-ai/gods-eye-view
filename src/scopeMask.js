...
let _viewer = null;
/**
 * OFF by default (Indonesia build): the circular scope treatment reads as
 * looking through an eye/telescope, and this build wants the plain full-frame
 * globe with the HUD still on top. The toggle (#scope-toggle) and the `sc=1`
 * share-link field both still turn it back on.
 */
let _enabled = false;
...
export function _resetScopeMaskForTest() {
  ...
  _container = null;
  _viewer = null;
  // Teardown restores the first-run default, which is OFF in this build.
  _enabled = false;
